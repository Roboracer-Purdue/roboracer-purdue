...
class SimpleDriver(Node):
    ...
    def control_loop(self):
        ...
        # READING SCAN DATA & COMPUTE SPEED AND STEERING
        # ------------------------------------------------------

        # Read LIDAR scan directly ahead -> 0 degree
        if self.get_scan_at_angle(0.0) > 0.8:
            # Forward speed if there is enough gap in front
            speed = 0.8
        else:
            speed = 0.0
        
        # Read LIDAR scan directly to the left -> 90.0 degree
        if self.get_scan_at_angle(90.0) > 0.8:
            # Steer left if there is too much gap to the left
            steering = 0.35 # Positive = Left turn
        else: 
            steering = -0.35 # Positive = Roght turn
        ...
        # Special logging function to log for logging at lower frequency
        self.log_throttled(0.5, f"Driving at {speed:.2f} m/s with steering of {steering:.2f}.")
