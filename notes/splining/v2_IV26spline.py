...
def compute_attributes(
    ...
    seg_dx = np.diff(x, append=x[0])
    seg_dy = np.diff(y, append=y[0])
    seg_len = np.sqrt(seg_dx**2 + seg_dy**2)

    # For each point, find curvature some distance ahead
    n = len(x)
    future_curvature = np.zeros(n)
    ...
    for i in range(n):
        ...
        future_curvature[i] = curvature[j]

    # Velocity based on future curvature, not current curvature
    velocity = max_speed / (
        1.0 + curvature_speed_gain * np.abs(future_curvature)
    )
    velocity = np.clip(velocity, min_speed, max_speed)

    yaw = (yaw + np.pi) % (2 * np.pi) - np.pi

    return yaw, curvature, velocity
...
def main():
    ...
    parser.add_argument("--lookahead-distance", type=float, default=0.8)
    ...
    yaw, curvature, velocity = compute_attributes(
        x,
        y,
        max_speed=args.max_speed,
        min_speed=args.min_speed,
        curvature_speed_gain=args.curvature_gain,
